...
#define NCAL 9
const char cal[NCAL][11] = {"1.8MHz","3.5MHz","7MHz","14MHz","28MHz","50MHz","70MHz","144MHz","432MHz"};
// dB coupling corrections 
//float Offset[NCAL] =  { 30,40,45,51,57,61,64,69,66};
float Offset[NCAL] =  { 33.6,38.8,45.2,51.0,57.3,61.8,64.5,69.6,69.0};
class ADCread{
private:
  struct AD {
    float Reading;
...
#define DELAY_REF 10
  ...
  void GetInitCal(){
    ...
    for(i=0;i<NCAL;i++) EEPROM.get(4+(i*sizeof(float)),Offset[i]);
  }
public:
  void SaveCal(byte c){
    byte NewCalIndex;
